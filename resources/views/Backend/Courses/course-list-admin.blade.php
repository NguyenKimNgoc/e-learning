                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                <?php $n=1;?>
                                @foreach($listCourseAdmin as $courses)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($courses->status == 0)
                                                <form action="{{route('change-status',$courses->id)}}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="email" value="{{$courses->email}}">
                                                    <input type="hidden" name="c_title" value="{{$courses->c_title}}">
                                                    <input type="hidden" name="route" value="{{route('course-list')}}">
                                                    <input type="hidden" name="full_name" value="{{$courses->full_name}}">
                                        ...
                                        <td class="text-center">
                                            ...
                                            @if(Auth::user()->id == $courses->c_author_id)
                                            ...
                                            @endif
                                            @if($countCustomer > 20)
                                                <a href="{{route('send-mail-to-customer',$courses->id)}}"
                                                   class="btn btn-xs btn-primary"  data-toggle="tooltip" data-placement="top" title="Gửi mail cho khách hàng"><i class="fa fa-envelope"></i></a>
                                            @endif
                                            @if($countRegister > 20)
                                                <a href="{{route('send-mail-to-register',$courses->id)}}" class="btn btn-xs btn-warning"  data-toggle="tooltip" data-placement="top" title="Gửi mail cho khách đăng ký"><i class="fa fa-paper-plane"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                                <tr>
                                    @if(count($listCourseAdmin) == 0 )
                                        <td colspan="5" class="text-center"><em>Không có Khóa Học nào</em></td>
                                    @endif
                                </tr>
                                <tr>
                                    <td colspan="7">{!! $listCourseAdmin->links() !!}</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
