    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form role="form" action="{{route('blog-edit',['id'=>$editBlog->id,'slug_blog'=>$editBlog->slug_blog])}}" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="box-body">
                        <div class="col-md-6">
                            <div class="form-group {{ !empty($errors->first('blog_title')) ? 'has-warring' : '' }}">
                                ...
                                <input type="text" class="form-control" name="blog_title"
                                       value="{!! old('blog_title',$editBlog->blog_title) !!}"
                            ...
                            <div class="form-group {{ !empty($errors->first('images_blog')) ? 'has-warring' : '' }}">
                                ...
                                <input type="file" class="form-control"  onchange='openFile(event)' name="images_blog"
                                       @if($errors->has('images_blog')) autofocus @endif value="{!! old('images_blog',$editBlog->images_blog) !!}"
                                       placeholder="Nhập hình ảnh...">
                                <img id="output" src="{{url('/')}}{{$editBlog->images_blog}}" style="margin-top: 15px ; width: 100%" />
                        ...
                        <div class="col-md-6">
                            <div class="form-group {{ !empty($errors->first('sub_title')) ? 'has-warring' : '' }}">
                                ...
                                <textarea name="sub_title" id="" cols="10" rows="5" class="form-control">{{old('sub_title',$editBlog->sub_title)}}</textarea>
                            ...
                            <div class="form-group {{ !empty($errors->first('cate_group_id')) ? 'has-warring' : '' }}">
                                ...
                                <select name="cate_group_id" id="input" class="form-control select-industries"
                                    ...
                                    @foreach($listCateGroup as $cateGroup)
                                        <option value="{{$cateGroup->id}}" {{$cateGroup->id == old('cate_group_id',$editBlog->cate_group_id) ? "selected" : ''}}>{{$cateGroup->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group {{ !empty($errors->first('tagID')) ? 'has-warring' : '' }}">
                                ...
                                <select name="tagID[]" id="input" class="form-control select-tags"
                                    ...
                                    @foreach($listTags as $tags)
                                        <?php
                                        foreach ($listTagsBlog as $blogTags){
                                            if (isset($blogTags->blog_tags) && $tags->id == $blogTags->blog_tags) {
                                                $selected = 'selected';
                                            } else {
                                                $selected = '';
                                            }
                        ...
                        <div class="col-md-12">
                            <div class="form-group {{ !empty($errors->first('content_blog')) ? 'has-warring' : '' }}">
                                ...
                                <textarea id="editor1" name="content_blog" rows="10" cols="80" placeholder="Nhập nội dung...">{!! $editBlog->content_blog !!}</textarea>
                    ...
                    <div class="box-footer">
                        ...
                        <button type="submit" class="btn btn-success"><i class="fa fa-user-plus"></i> Cập Nhật Tin Tức
                        </button>
                    </div>
                </form>
            </div>
